...
export function FavoriteViews({ title, type }) {
  ...
  // Combinamos favoritos con sus datos reales (PBI o Layout) respetando el orden de 'favorites'
  ...
  const handleDragEnd = (event) => {
    ...
    const oldIndex = displayData.findIndex((i) => i.favTableId === active.id);
    const newIndex = displayData.findIndex((i) => i.favTableId === over.id);
    const newArray = arrayMove(displayData, oldIndex, newIndex);
      ...
      {displayData.length > 0 && (
        <>
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={displayData.map(i => i.favTableId)} strategy={rectSortingStrategy}>
  ...
  return (
    ...
    >
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <p className="text-gray-600 text-sm flex-grow mb-4">{item.description || "Sin descripción."}</p>
