  ...
  return (
    <article
      ...
      className={`group overflow-hidden rounded-[30px] border border-[#dce3e8] bg-[linear-gradient(180deg,#ffffff_0%,#f8fafb_100%)] shadow-[0_18px_36px_-24px_rgba(31,41,55,0.14)] transition-all duration-300 ${
        isDragging
          ? "z-50 scale-[1.03] rotate-[1deg] opacity-90 shadow-[0_35px_60px_-15px_rgba(0,0,0,0.15)] ring-4 ring-[#255f82]/5"
          : "hover:shadow-[0_20px_45px_-15px_rgba(0,0,0,0.1)]"
      }`}
    >
      <div
        {...dragProps}
        className="flex cursor-grab items-center justify-between border-b border-[#eef1f5] bg-[#fbfcfd] px-5 py-4 active:cursor-grabbing"
      >
      ...
      <div className="px-6 py-6">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            <h2 className="mt-2 truncate text-2xl font-bold tracking-tight text-[#1e293b] transition-colors group-hover:text-[#255f82]">
              {item.name}
            </h2>
      ...
      <div className="flex items-center justify-between border-t border-[#eef1f5] bg-[#f9fbfc] px-5 py-5">
        <div className="scale-110">
          <FavoriteHeart type={type} objectId={item.id} />
        </div>

        <div className="flex items-center gap-3">
          <Button onClick={() => onView(item)}>Ver detalle</Button>
